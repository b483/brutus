...
struct empire_object_t empire_objects[MAX_EMPIRE_OBJECTS];
...
void empire_load_editor(int empire_id, int viewport_width, int viewport_height)
{
    ...
    empire_data.viewport_width = viewport_width;
    empire_data.viewport_height = viewport_height;
    if (our_city) {
...
{
    buffer_write_i32(buf, empire_data.scroll_x);
    buffer_write_i32(buf, empire_data.scroll_y);
}

void empire_load_state(struct buffer_t *buf)
{
    empire_data.scroll_x = buffer_read_i32(buf);
    empire_data.scroll_y = buffer_read_i32(buf);
}
